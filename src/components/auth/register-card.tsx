...
import { INITIAL_AUTH_FORM_STATE } from "@/lib/auth/auth-form-state";

type RegisterCardProps = {
  signupClosed?: boolean;
};

export function RegisterCard({ signupClosed = false }: RegisterCardProps) {
  ...
  return (
    <Card className="crm-panel w-full max-w-[33rem] rounded-[2rem] px-6 py-5 sm:px-9 sm:py-6">
      ...
      <form action={formAction} autoComplete="off" className="mt-6 space-y-5">
        ...
        <TextField
          ...
          type="text"
        />
        {state.fieldErrors?.fullName ? <p className="px-1 text-sm text-red-300">{state.fieldErrors.fullName}</p> : null}
        ...
        <Button className="mt-2" disabled={pending || signupClosed} type="submit">
          {signupClosed ? "Signup Closed" : pending ? "Creating account..." : "Register Now >"}
        </Button>
      </form>
      ...
      <Button className="mt-7" disabled icon={<GoogleMark />} variant="secondary">
        Continue with Google
      </Button>

      <p className="mt-10 text-center text-sm text-slate-400">
        Already have an account?{" "}
        <Link className="font-medium text-blue-300 transition hover:text-blue-200" href="/login">
          SignIn
        </Link>
      </p>
    </Card>
